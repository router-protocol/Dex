...
use swap::asset::{Asset, AssetInfo, PairInfo};
use swap::pair::SimulationResponse;
use swap::querier::{query_pair_info, reverse_simulate, simulate};
use swap::router::{
    ConfigResponse, Cw20HookMsg, ExecuteMsg, InstantiateMsg, MigrateMsg, QueryMsg,
    SimulateSwapOperationsResponse, SwapOperation,
};
use swap::util::migrate_version;

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:swap-router";
...
pub fn instantiate(
    ...
    CONFIG.save(
        deps.storage,
        &Config {
            swap_factory: deps.api.addr_canonicalize(&msg.swap_factory)?,
        },
    )?;
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        swap_factory: deps
            .api
            .addr_humanize(&state.swap_factory)?
            .to_string(),
    };
...
fn simulate_swap_operations(
    ...
    let swap_factory = deps.api.addr_humanize(&config.swap_factory)?;
