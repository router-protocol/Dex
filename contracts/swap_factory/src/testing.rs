...
use swap::mock_querier::{mock_dependencies, WasmMockQuerier};
...
fn create_pair() {
    ...
    deps = init(deps);
    deps.querier
        .with_swap_factory(&[], &[("uusd".to_string(), 6u8)]);
    let assets = [
        Asset {
...
fn fail_to_create_pair_with_unknown_token() {
    ...
    deps.querier
        .with_swap_factory(&[], &[("uluna".to_string(), 6u8)]);
